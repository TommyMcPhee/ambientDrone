...
public:
	...
	const float phaseIncrement = (float)(limit * wavetableSize) * 0.5 / (float)sampleRate;
	int bankIndex = 0;
	...
	float progress = 0.0, droneAmplitude = 0.0, dronePhase = 0.0, droneSample = 0.0, activeOscillators, keytotal = 0.0, mouseX = 0.5, mouseY = 0.5;
	array<float, 2> lastSample, sample;
